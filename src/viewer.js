 ...
 *   1. one mesh per part, so a full body is under forty draw calls;
 *   2. render on demand — frames are only drawn when something changed.
 */
import * as THREE from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { RoomEnvironment } from 'three/addons/environments/RoomEnvironment.js';
import { PARTS } from './data.js';
import { BUILDERS, TWO_SIDED, BODY } from './geometry.js';
...
export function createViewer(canvas) {
  ...
  const parts = new Map();          // id → { mesh, material, part, box, sphere }
  let pickable = [];
  let selected = null;
  let hovered = null;
  let isolate = false;
  ...
  /* ── building ─────────────────────────────────────────────── */
  api.build = async function build(onProgress) {
    for (let i = 0; i < PARTS.length; i++) {
      const part = PARTS[i];
      const make = BUILDERS[part.build];
      if (!make) continue;
      ...
      const material = new THREE.MeshStandardMaterial({
        ...
        roughness: part.sys === 'skeletal' ? 0.62 : 0.42,
        metalness: 0,
   ...
   */
  api.loadModels = async function loadModels() {
    let manifest;
    try {
    ...
    for (const [file, map] of Object.entries(manifest.files || {})) {
      let gltf;
  ...
  function refreshVisibility() {
    ...
    for (const rec of parts.values()) {
      const sysOff = hiddenSystems.has(rec.part.sys);
  ...
  api.setSystemVisible = (sys, on) => {
    ...
    if (!on && selected && parts.get(selected)?.part.sys === sys) api.select(null);
  ...
  api.setClip = (axis, amount) => {
    clipAxis = CLIP[axis] ? axis : 'z';
    clipAmount = clamp(amount, 0, 1);
    const spec = CLIP[clipAxis];
    clipPlane.normal.set(...spec.normal);
  ...
  api.select = (id, { focus = false } = {}) => {
    ...
    selected = id || null;
    if (selected) {
      const sys = parts.get(selected).part.sys;
  ...
  function moveTo(position, target, ms = 700) {
