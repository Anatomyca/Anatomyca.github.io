/**
 * Every part of the body is generated in code — no model downloads, no
 * licence tangles, and the whole atlas stays a few hundred kilobytes.
 * Shapes are anatomically placed rather than anatomically exact: the aim
 * is a teaching model you can recognise and navigate, not a dissection.
 *
 * Units are metres on a body 1.75 m tall, standing, facing +Z.
 *   +X  the model's left        +Y  up        +Z  towards the viewer
 *
 * Drop-in replacement: see models/README.md to swap any part for a real
 * mesh from a GLB file without touching this file.
 */
...
function capsule(a, b, rA, rB = rA, radial = 14) {
...
function flatten(g, at, sx = 1, sz = 0.7) {
  g.translate(0, -at, 0);
  g.scale(sx, 1, sz);
  g.translate(0, at, 0);
  return g;
...
function wobble(x, y, z, freq, seed) {
  return (
 ...
 */
function blob({ at = [0, 0, 0], scale = [0.05, 0.05, 0.05], seg = [40, 26], amp = 0, freq = 8, seed = 1, warp = null }) {
...
function tube(points, r, tubular = 48, radial = 9) {
...
function dent(v, cx, cy, cz, radius, depth) {
